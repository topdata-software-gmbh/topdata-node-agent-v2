...
- `serve` starts one goroutine per shop (log tail + disk scanner) and blocks in `http.ListenAndServe`; it `log.Fatal`s if discovery errors, and fails at startup if the configured shops directory doesn't exist. It registers three endpoints: `/healthz` (unauthenticated liveness probe, always `200 OK` if the process is listening), and two Basic-Auth endpoints `/metrics` and `/disk-eaters` (ranked disk-growth listing; `?shop=`, `?top=`, `?by=rate|size`, `?format=json|text|markdown`, with human-readable sizes; defaults to JSON, or follows `Accept` (`text/plain`/`text/markdown`) when no `format` is given). It also registers `/info` (Basic Auth; `?format=json|text|markdown` with the same `Accept` fallback), reporting `version`, `uptime`, `started_at`, `listen_address`, `shops_root`, and `shops_total`.
