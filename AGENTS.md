...
- `serve` starts one goroutine per shop (log tail + disk scanner) and blocks in `http.ListenAndServe`; it `log.Fatal`s if discovery errors, and fails at startup if the configured shops directory doesn't exist. It registers four endpoints: `/healthz` (unauthenticated liveness probe, always `200 OK` if the process is listening), and Basic-Auth endpoints `/metrics`, `/disk-eaters` (ranked disk-growth listing; `?shop=`, `?top=`, `?by=rate|size`, `?format=json|text|markdown`, with human-readable sizes; defaults to JSON, or follows `Accept` (`text/plain`/`text/markdown`) when no `format` is given) and `/critical-errors` (recent critical error lines per shop from an in-memory 100-line-per-shop ring buffer fed by the log tailer; `?shop=`, `?limit=`, `?format=json|text|markdown` with the same `Accept` fallback; history resets on restart, payload carries `agent_started_at`). It also registers `/info` (Basic Auth; `?format=json|text|markdown` with the same `Accept` fallback), reporting `version`, `uptime`, `started_at`, `listen_address`, `shops_root`, and `shops_total`.
